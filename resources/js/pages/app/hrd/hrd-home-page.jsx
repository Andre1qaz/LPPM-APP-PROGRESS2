            ...
            <div className="w-full max-w-6xl mx-auto p-6 pt-0 pb-24 space-y-8">
                ...
                <div className="space-y-4">
                    {notifications.map((item) => {
                        ...
                        // Komponen Konten Kartu (Reusable)
                        const CardInner = (
                            <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                                {/* KIRI: Info Utama */}
                                <div className="flex items-center gap-4 w-full md:w-2/3">
                                    <div className={`shrink-0 h-12 w-12 flex items-center justify-center rounded-full ${
                                        item.status === 'Selesai' ? 'bg-green-600' : 'bg-black'
                                    } text-white`}>
                                        {item.status === 'Selesai' ? <CheckCircle2 className="w-6 h-6" /> : <FileText className="w-6 h-6" />}
                                    </div>
